<!-- location-guide -->
<section class="location-guide">
    <div class="container">
        <div class="section-header">
            <h2>展区位置</h2>
        </div>
        <div class="section-content">
            <article class="booth">
                <figure class="location-map">
                    <img src="/mobile/assets/images/main/bg_guide_maps.png" class="img-fluid" alt="">
                </figure>
                <div class="head">
                    <ul>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_top_left.png" class="img-fluid" alt="">
                        </li>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_top.png" class="img-fluid" alt="">
                            <ul>
                                <li>
                                    <a href="#">
                                        <img src="/mobile/assets/images/main/eng/img_link02.png" class="img-fluid" alt="">
                                    </a>
                                </li>
                                <li>
                                    <img src="/mobile/assets/images/main/img_space_top.png" class="img-fluid" alt="">
                                </li>
                                <li>
                                    <a href="#">
                                        <img src="/mobile/assets/images/main/eng/img_link01.png" class="img-fluid" alt="">
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_top_right.png" class="img-fluid" alt="">
                        </li>
                    </ul>
                </div>
                <div class="body">
                    <ul>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_body_left.png" class="img-fluid" alt="">
                        </li>
                        <li>
                            <a href="#">
                                <img src="/mobile/assets/images/main/eng/img_link03.png" class="img-fluid top-4" alt="">
                            </a>
                            <img src="/mobile/assets/images/main/img_space_left_body_middle.png" class="img-fluid" alt="">
                            <a href="#">
                                <img src="/mobile/assets/images/main/eng/img_link04_1.png" class="img-fluid top-4" alt="">
                            </a>
                        </li>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_center.png" class="img-fluid" alt="">
                        </li>
                        <li>
                            <a href="#">
                                <img src="/mobile/assets/images/main/eng/img_link08_2.png" class="img-fluid" alt="">
                            </a>
                            <img src="/mobile/assets/images/main/img_space_right_body_middle.png" class="img-fluid" alt="">
                            <div class="cell-content">
                                <a href="#">
                                    <img src="/mobile/assets/images/main/eng/img_link07_1.png" class="img-fluid top-3" alt="">
                                </a>
                                <img src="/mobile/assets/images/main/img_space_right_body_center.png" class="img-fluid" alt="">
                                <a href="#">
                                    <img src="/mobile/assets/images/main/eng/img_link07_2.png" class="img-fluid" alt="">
                                </a>
                            </div>
                        </li>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_body_right.png" class="img-fluid" alt="">
                        </li>
                    </ul>
                </div>
                <div class="foot">
                    <ul>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_bottom_left.png" class="img-fluid" alt="">
                        </li>
                        <li>
                            <div class="cell-content">
                                <div>
                                    <a href="#">
                                        <img src="/mobile/assets/images/main/eng/img_link05_2.png" class="img-fluid" alt="">
                                    </a>
                                    <img src="/mobile/assets/images/main/img_space_bottom_foot05_center.png" class="img-fluid" alt="">
                                    <a href="#">
                                        <img src="/mobile/assets/images/main/eng/img_link05_1.png" class="img-fluid" alt="">
                                    </a>
                                </div>
                                <img src="/mobile/assets/images/main/img_space_bottom_foot_center.png" class="img-fluid" alt="">
                                <a href="#">
                                    <img src="/mobile/assets/images/main/eng/img_link06_1.png" class="img-fluid" alt="">
                                </a>
                            </div>
                            <img src="/mobile/assets/images/main/img_location_bottom.png" class="img-fluid" alt="">
                        </li>
                        <li>
                            <img src="/mobile/assets/images/main/img_location_bottom_right.png" class="img-fluid" alt="">
                        </li>
                    </ul>
                </div>
            </article>
            <article class="guide-list">
                <div class="table-wrap">
                    <table class="table">
                        <tbody>
                            <tr>
                                <th>1H</th>
                                <td><a href="#">服务贸易展区</a></td>
                            </tr>
                            <tr>
                                <th>2H</th>
                                <td><a href="#">汽车展区</a></td>
                            </tr>
                            <tr>
                                <th>3H</th>
                                <td><a href="#">智能及高端装备展区</a></td>
                            </tr>
                            <tr>
                                <th>4.1H</th>
                                <td><a href="#">智能及高端装备展区</a></td>
                            </tr>
                            <tr>
                                <th>5.1H</th>
                                <td><a href="#">消费及电子家电展区</a></td>
                            </tr>
                            <tr>
                                <th>5.2H</th>
                                <td><a href="#">韩国宣传馆</a></td>
                            </tr>
                            <tr>
                                <th>6.1H</th>
                                <td><a href="#">服装服饰及日用消费品展区</a></td>
                            </tr>
                            <tr>
                                <th>7.1H</th>
                                <td><a href="#">医疗器械及医药保健展区</a></td>
                            </tr>
                            <tr>
                                <th>7.2H</th>
                                <td><a href="#">食品及农产品展区</a></td>
                            </tr>
                            <tr>
                                <th>8.2H</th>
                                <td><a href="#">食品及农产品展区</a></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </article>
        </div>
    </div>
</section>